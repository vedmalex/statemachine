...
## 11. IMPLEMENT amendment — §3.7 DOCUMENTED_GAP_IDS restored (F-PF-1)

**DA IMPLEMENT-exit revision (F-PF-1).** `src/sim/capabilities.ts` `DOCUMENTED_GAP_IDS` had been wrongly widened from the §3.7-frozen FOUR ids to TWELVE, neutering the ADR-8 mandatory coverage gate (`coverage.ts` `computeCoverage`) on ~1/3 of the registry (only ~27 of 39 CapabilityIds enforced). This amendment RESTORES the gate to the frozen-minimal set plus three explicitly-amended structural gaps = **SEVEN ids total**, deep-equal pinned by `src/tests/sim/capabilities_gap_pin.test.ts` so it cannot silently re-widen.

Final `DOCUMENTED_GAP_IDS` (sorted, 7):
```
error.action-throw                  # FROZEN-MINIMAL: string-method-resolved callAction throw swallowed (:1774)
error.guard-throw                   # FROZEN-MINIMAL
error.recovery.abortOnExitError     # FROZEN-MINIMAL
queue.depth-bound.max-transition    # FROZEN-MINIMAL: dormant :303 throw (no non-fault scenario reaches it)
event.onError                       # AMENDED STRUCTURAL: dispatched UN-AWAITED (:2037, §4.1); ErrorHandler<T> returns void
event.onSuccess                     # AMENDED STRUCTURAL: engine NEVER dispatches onSuccess at runtime (only (de)serialized :956/:982/:2688)
queue.internal-before-external      # AMENDED STRUCTURAL: boundary-only content trace ⇒ queue.internal===0 on EVERY captured frame
```

**The three AMENDED structural gaps** (beyond the original frozen 4) are genuinely unreachable in v1, NOT merely un-driven:
- `queue.internal-before-external` — EMPIRICALLY confirmed: the engine drains the internal queue fully before each external and before the macrostep settles, so every captured frame (the Adapter-seam writes DURING the drain AND the settle-boundary frame) reads `queue.internal===0`. An external fire driving a chain of `delay:0` internal raises never yields a frame with `internal>0`. Observing it would require sampling INSIDE the engine's private drain loop, which ADR-7 c8 forbids (zero core-ABI).
- `event.onSuccess` — the engine ONLY (de)serializes `event.onSuccess` (state_machine.ts:956/982/2688); it is dispatched NOWHERE in the transition flow. A dormant config field.
- `event.onError` — dispatched UN-AWAITED (state_machine.ts:2037) and only on an awaited-callback throw; `ErrorHandler<T>` returns `void` (§4.1 OUT OF SCOPE) so it carries no tracked async work observable in the content trace.

**FIVE ids the wrongly-widened set excluded are now COVERED** (removed from the gap set): `queue.backpressure.overflow`, `timer.transitionTimeout`, `timer.resume`, `persistence.serialize`, `persistence.deserialize`. They are exercised by wire-time coverage-runner drive paths in `coverage.ts` (`driveOverflowFrame` / `driveTransitionTimeoutFrame` / `drivePostRestoreFrame`) keyed on `CoverageScenario` envelope intents (`overflow` / `transitionTimeout` / `snapshotRestore`), with scenarios in `src/sim/scenarios/backpressure-timeout.ts` (overflow + transitionTimeout) and `src/sim/scenarios/persistence.ts` (snapshot/restore):
- `queue.backpressure.overflow` — `maxQueueDepth:2` + a flood of 8 concurrent fires; the `(max+1)`-th enqueue rejects SYNCHRONOUSLY (state_machine.ts:234) → `queue-overflow` errorClass frame.
- `timer.transitionTimeout` — a hanging-`onTransition` fire under `transitionTimeout:2`; a deterministic virtual-clock advance + scheduler-process pump fires the timeout `Promise.race` leg (state_machine.ts:1789/1798) → `transition-timeout` errorClass frame. Discriminated structurally by `phase:'action' && action present && cause===undefined` (a normal action-throw carries a `cause`; an injected fault is caught by identity), NEVER by `e.message`. NOTE: this CORRECTS the build-plan/§3.3 'NO transition/state' phrasing — that holds only for the swallowed invoke-action leg; the OBSERVABLE harness-boundary timeout reject carries `transition`/`state` but no `cause`, and `cause===undefined` is the verified discriminator.
- `timer.resume` / `persistence.serialize` / `persistence.deserialize` — a public `saveState(mem)` + `restoreState(mem)` round-trip (`restoreState` re-validates the composite, re-applies history, and re-arms invoke timers via `resumeTimers()` state_machine.ts:740) → ONE `synthetic:'post-restore'` frame.

**Gate result after the fix:** 32 covered / 39 total, 7 gapped, `sim:coverage` exit 0 (was 27 covered / 12 gapped). The gate's teeth are re-proven by `capabilities_gap_pin.test.ts`: removing any one covering scenario ⇒ its id reported `uncovered` + exit 1.

(See also §1 / §5 F-PF-2: the public `SimOptions.invariants` Safety path is now wired — `Simulator`/`runSimulation` evaluate the blind `runSafety` registry at each step boundary and populate `SimResult.ok`/`violation`; the repro-codegen emitted test replays through the public `runSimulation` with the frozen `INVARIANTS` registry, closing the DoD-9b coupling.)
